 ...
 if(! class_exists('AutoReplaceBrokenLinksForYoutube')){
	class AutoReplaceBrokenLinksForYoutube {
		...
		public function List_Youtube_Broken_Links() {
			...
			$total_items = $wpdb->get_var( "SELECT COUNT(*) FROM ".$wpdb->prefix . "auto_replace_broken_links_for_youtube" );

			// Total page number
			$total_pages = ceil( $total_items / $per_page );

			// Current page number
			$current_page = isset( $_COOKIE['paged'] ) ? absint( $_COOKIE['paged'] ) : setcookie('paged', 1, time() + (86400 * 1), "/");

			setcookie('paged', 1, time() + (86400 * 1), "/");

			// Offset query select
			$offset = ( $current_page - 1 ) * $per_page;
            */
			$query = "SELECT IdPost, DateOperation, TitlePost, UrlPermalinkPost, WasSent, Action FROM " . $wpdb->prefix . "auto_replace_broken_links_for_youtube WHERE (YEAR(DateOperation)=YEAR(CURDATE())) ORDER BY DateOperation DESC LIMIT 20";			
...
/*
				...
				if ( $total_pages > 1 ) {
					$current_url = esc_url( remove_query_arg( 'paged' ) );
					...
					echo paginate_links( array(
						...
						'mid_size' => 2,
						'prev_next' => true,
						'prev_text' => __('&laquo; Previous','auto-replace-broken-links-for-youtube'),
						'next_text' => __('Next &raquo;','auto-replace-broken-links-for-youtube'),
						'type' => 'plain',
						'add_args' => false,
						'add_fragment' => ''
					) );
				...
				setcookie($current_page+1, 1, time() + (86400 * 1), "/");
*/
			}	
