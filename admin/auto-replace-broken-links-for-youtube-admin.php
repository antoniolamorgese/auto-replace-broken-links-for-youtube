	...
	class AutoReplaceBrokenLinksForYoutube {
		...
		public function auto_replace_broken_links_for_youtube_create_admin_page() {
			...
			<div class="wrap">
				...
				<p><?php _e('Youtube Broken Link Checker hide all broken links youtube videos to improve SEO','auto-replace-broken-links-for-youtube'); ?>.</p>
				<?php settings_errors(); ?>
		...
		public function List_Youtube_Broken_Links() {
			global $wpdb;
			$query = "SELECT IdPost, DateOperation, TitlePost, UrlPermalinkPost, WasSent, Action FROM " . $wpdb->prefix . "auto_replace_broken_links_for_youtube WHERE (YEAR(DateOperation)=YEAR(CURDATE())) ORDER BY DateOperation DESC LIMIT 50";			
			$results = $wpdb->get_results($query);

			if(!empty($results)) {
				$title_table  = '<hr><h3 style="color:black;">'.__('Youtube Broken Links','auto-replace-broken-links-for-youtube').'...</h3><br>';
