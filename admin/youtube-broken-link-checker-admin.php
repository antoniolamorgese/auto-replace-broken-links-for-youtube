	...
	class YoutubeBrokenLinkChecker {
		...
		public function sent_email_every_3_callback() {
			printf(
				...
				isset( $this->youtube_broken_link_checker_options['sent_email_every_3'] ) ? esc_attr( $this->youtube_broken_link_checker_options['sent_email_every_3']) : ''
			);
		}

		public function List_Youtube_Broken_Links() {
			global $wpdb;
			$results = $wpdb->get_results("SELECT IdPost, DateOperation, TitlePost, UrlPermalinkPost, WasSent FROM " . $wpdb->prefix . "_youtube_broken_link_checker WHERE (YEAR(DateOperation)=YEAR(CURDATE())) ORDER BY DateOperation DESC LIMIT 50");
			...
			if(!empty($results)) {
				...
				$table .= '<thead>';
				$table .= '<tr>';
				$table .= '<th><b style="color:blue;">'.__('ID Post', 'youtube-broken-link-checker').'</b></th>';
				$table .= '<th><b style="color:blue;">'.__('Detected', 'youtube-broken-link-checker').'</b></th>';
				$table .= '<th><b style="color:blue;"'.__('>Post', 'youtube-broken-link-checker').'</b></th>';
				$table .= '<th><b style="color:blue;">'.__('URL Post', 'youtube-broken-link-checker').'</b></th>';
				$table .= '<th><b style="color:blue;">'.__('Sent', 'youtube-broken-link-checker').'</b></th>';
				$table .= '</tr>';
				$table .= '</thead>';
				$table .= '<tbody>';
				...
				foreach ($results as $row) {
					$table .= '<tr>';
					$table .= '<td>' . $row->IdPost . '</td>';
					$table .= '<td>' . $row->DateOperation . '</td>';
					$table .= '<td>' . $row->TitlePost . '</td>';
					$table .= '<td>' . $row->UrlPermalinkPost . '</td>';
					$table .= '<td>' . $row->WasSent . '</td>';
					$table .= '</tr>';
				}
