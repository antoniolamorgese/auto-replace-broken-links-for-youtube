				...
				jQuery(document).ready(function(){
					 ...
					 */	
				 	function searchYouTubeVideos(apiKey, query, currentElement__wrapper) {
						...
						xhr.open("GET", urlSearchVideoYoutube + params, true);
						xhr.send();
						xhr.onload = function(){
							if (xhr.status === 200){
								...
								if(video_ID != undefined){
									...
									var titleYoutubeVideo = data.items[0].snippet.title;

									<?php if(wp_is_mobile()){ ?>
										var urlYoutubeThumbnail = data.items[0].snippet.thumbnails.medium.url;
									...
									<?php } else { ?>
										var urlYoutubeThumbnail = data.items[0].snippet.thumbnails.high.url;
					...
					function verifyYoutubeBrokenlinkChecker( APIKey, youtubeUrl, action, currentElement__wrapper ) {
						...
						xhr2.open("GET", urlGoogleApis + params, true);
						xhr2.send();
						xhr2.onload = function(){
							if (xhr2.status === 200){
								...
								if(data.pageInfo.totalResults != 'undefined'){
									if(data.pageInfo.totalResults == 0){
										 ...
										 */	
										<?php //insert_youtube_broken_link_in_mysql_table(get_the_ID(), $Action); ?>
