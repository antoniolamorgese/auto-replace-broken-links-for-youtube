		...
		?>
			...
			<script>
				jQuery(document).ready(function(){
						...
						xhr.onload = function(){
							if (xhr.status === 200){
								...
								if(video_ID != undefined){
									...
									//CodeHTMLButton = '<button onclick="'+urlYoutube+'"><?php //_e('WATCH VIDEO NOW','auto-replace-broken-links-for-youtube'); ?></button>'; 
									<?php if(wp_is_mobile()){ ?>
									...
									jQuery('.wp-block-embed__wrapper').html(CodeHTMLthumbnail);
									jQuery('figcaption.wp-element-caption').html(CodeHTMLCaptionThumbnail);
					...
					/**
					 * Startup Auto Replace Broken Links For Youtube
					...
					if(is_page() || is_single()) { 
						if(isset($Google_Youtube_API_Key)) {
							if(strlen($Google_Youtube_API_Key) > 0) { 
								?>
									//var elements = jQuery('.wp-block-embed__wrapper');
									//for (var i = 0; i < elements.length; i++) {
										//var currentElement = jQuery(elements[i]);
