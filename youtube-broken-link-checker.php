 ...
 */
$table_name = $wpdb->prefix . 'youtube_broken_link_checker';
...
if ($total_rows > 0) {
	if(isset($youtube_broken_link_checker_options['sent_email_every_3'])) {
		...
		if(!isset($Sent_Email_Every)) $Sent_Email_Every = 1;
	} 
}
...
if(!function_exists('send_html_email_youtube_broken_link_checker')){
	function send_html_email_youtube_broken_link_checker($action) {
		global $youtube_broken_link_checker_options;
		global $post;
		...
		if(function_exists('insert_youtube_broken_link_in_mysql_table')) insert_youtube_broken_link_in_mysql_table(get_the_ID(), $youtube_broken_link_checker_options['action_2']);
		if(function_exists('get_youtube_broken_link_in_mysql_table')){
			if(function_exists('update_youtube_broken_link_in_mysql_table')){
...
if(!function_exists('youtube_broken_link_checker_get_key_frase_current_post')){
	function youtube_broken_link_checker_get_key_frase_current_post(){
		...
		if (!empty($keyword)) {
			return preg_replace('/[^A-Za-z \?!]/','',$keyword);
		} else {
		...
		?>
			...
			<script>
				jQuery(document).ready(function(){
					 ...
					 */	
				 	function searchYouTubeVideos(apiKey, query) {
						...
						xhr.onload = function(){
							if (xhr.status === 200){
								...
								if(video_ID != undefined){
									urlYoutube = "window.location.href='https://www.youtube.com/watch?v="+video_ID+"';";
									CodeHTMLButton = '<button onclick="'+urlYoutube+'"><?php _e('WATCH VIDEO NOW','youtube-broken-link-checker'); ?></button>'; 
									jQuery('.wp-block-embed__wrapper').html(CodeHTMLButton);
									jQuery('figcaption.wp-element-caption').hide('fast');
									<?php if($Enable_Email_Notifications == 'YES') 
										send_html_email_youtube_broken_link_checker('Replace Hide'); 
									?>
								} else {
									...
									jQuery('figcaption.wp-element-caption').hide('fast');
									<?php if($Enable_Email_Notifications == 'YES') 
										send_html_email_youtube_broken_link_checker('Hide Video'); 
									?>
								}
							} else {
								jQuery('figure.wp-block-embed').hide('fast'); 
							...
							if (xhr.status === 200){
								...
								if(data.pageInfo.totalResults > 0){
									...
									if(action === 'Hide Video') {
										...
										searchYouTubeVideos(apiKey, querySearch);
									}
								}
							} else {
...
if(!function_exists('drop_table_youtube_broken_link_checker')) {
	...
	register_deactivation_hook( __FILE__, 'drop_table_youtube_broken_link_checker' );
}
