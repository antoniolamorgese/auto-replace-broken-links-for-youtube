 ...
 * Description:        Youtube Broken Link Checker monitors and checks all internal and external links on your site for broken youtube links. Hide all links to bad youtube videos to improve SEO and user experience. Start <a href="options-general.php?page=youtube-broken-link-checker">Youtube Broken Link checker Settings</a>.
	...
	function youtube_broken_link_checker_add_Code_html_in_tag_body() {
		...
		if($Enable_Email_Notifications === 'YES') {
			try {
				...
				if(isset($to)) {
					$subject = 'Youtube Broken Link DETECTED';
					$message = "Youtube Broken Link Checker has detected new broken links on your site.
								here's the list:
					
								Post : " . get_the_title() . "
								URL Post : " . get_the_permalink() . "
								URL Youtube: " . $ytUrl;
					if(is_page()) {
